...
import { CSVHeaderType, CSVWriter, CSVRecordsType } from '../csv/csvWriter';
import { IUploaderList } from '../csvUploader/CSVUploaderManager';
import CSVUploader from '../csvUploader/csvUploader';
export type UserRecordsItemRecord = {
	no: number;
	email: string;
	first_name: string;
	last_name: string;
...
export class FindFullUserJob extends BaseJob {
	...
	override async task() {
		try {
			const records = await this.userRep.readAll();
